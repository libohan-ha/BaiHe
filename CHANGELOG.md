...
3.bug：搜索的时候触发：
article.service.js 第 143-146 行，当前端传入 sort=latest 时，代码直接将 latest 作为 Prisma 的排序字段，但 Article 模型中没有 latest 字段，导致 Prisma 报错。
解决
...
8.你看这个第一个图片的展示有点不对劲啊。不像其他几张是填满容器的，而且文章详情页面图片的上部分都没了，将人物放到中间。可能是用户上传的图片格式、大小不一样，我们需要处理一下
完成
